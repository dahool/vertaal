...
CMD = 'java -jar yuicompressor-2.4.7.jar --nomunge --type %(type)s -o %(file)s --charset utf-8 %(file)s'

def minimizejs(path):
    for root, subf, files in os.walk(path):
        for fs in files:
            if fs in JS_FILES:
                minfile(os.path.abspath(os.path.join(root, fs)).replace('\\','\\\\'))
...
if __name__ == "__main__":
    minimizejs(os.path.normpath('/workspace/cdn/media/app/vertaal/1.6.2/'))