...
class ChangeField(BaseMutation):
    ...
    def mutate(self, app_label, proj_sig):
        ...
        sql_statements = []
        for field_attr, attr_value in self.field_attrs.items():
            if field_attr == 'field_type':
                old_field_attr = old_field_sig[field_attr]
            else:
            ...
            if not old_field_attr == attr_value:
                try:
                    ...
                    elif field_attr == 'field_type':
                        new_field_sig = copy.copy(old_field_sig)
                        new_field_sig.pop('field_type')
                        field = create_field(proj_sig, self.field_name, attr_value, new_field_sig)
                        sql_statements.extend(evolver_func(model, field, self.initial))
                    else:
