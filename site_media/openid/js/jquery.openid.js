...
(function($) {
  $.fn.openid = function(opt) {
    ...
    var defaults = {
      ...
      */
      providers: [
        ...
        {
          name: 'Verisign',
          url: 'https://{username}.pip.verisignlabs.com/'
        },
        {
          name: 'Vidoop',
    ...
    var setCookie = function(value) {
      ...
      document.cookie = "openid_provider=" + value + "; expires=" + date.toGMTString() + 
    ...
    var readCookie = function(){
      ...
      for(i in c){
        if ((pos = c[i].indexOf("openid_provider=")) != -1) 
          return $.trim(c[i].slice(pos + 16));
      }
    };
    ...
    var showInputBox = function() {
      ...
      inputarea.empty().show().append('<span class="oidlabel">' + lbl + ':</span><br/><input id="' + INPUTID + '" type="text" ' +
        ' name="username_txt" class="input_'+gprovider.name+'" /><input type="submit" value="' + settings.txt.sign + '"/>');
